#include "buffer_structs_template.cl"
...
__constant uint padInt[4] = { 0x1 << 7, 0x1 << 15, 0x1 << 23, 0x1 << 31 };
__constant uint maskInt[4] = { 0x00000000, 0x000000FF, 0x0000FFFF, 0x00FFFFFF };
...
{ \
    ...
    for (int i = 1; i <= l && i <= 1; i++) { msg[padIntIndex + i] = 0; } \
...
// === Wrapper-Kernel für flache Buffers ===
__kernel void hash_main_flat(
...
)
{
    ...
    uint inp[inBufferSize];
    for (int j = 0; j < inBufferSize; j++) inp[j] = 0;

    for (int i = 0; i < len; i++) {
        uint b = (uint)input[off + i];
        inp[i >> 2] |= (b << ((i & 3) * 8));
    }

    uint digest[4];
    hash_private(inp, len, digest);
